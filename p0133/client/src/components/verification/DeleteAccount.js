...
class DeleteAccount extends React.Component {
  ...
  componentDidMount = () => {
    ...
    fetch(settings.defaultSettings.REACT_APP_API_URL + '/deleteAccount', { headers, method: 'POST', body: JSON.stringify({ token: token }) })
      .then(function (response) {
        return response.json()
      })
      .then(function (responseJ) {
        if (responseJ.message === 'deactivated') {
          document.getElementById('response').className = 'response_good_verified'
          document.getElementById('response').innerHTML = 'This account is deleted'
        }
      })
